    ...
    def __init__(self, background, links, links_color, brand):
        ...
        for i in self.links:
            ...
            for j in i:
                ...
                li.add_content(href)
                li._add_script("$('.nav-item').off().click(function(e){e.preventDefault();$.ajax({url:'http://localhost:8000'+$(this).find('a').attr('href'), success:function(data){$('.global').html(data);}});});")
